...
pub fn compare_files(folder: &Path, pinned_path: &Path, track_path: &Path) -> Vec<TrackDifference> {
    ...
    let Ok(track_workflow) = parse_file(&track_workflow_path) else {
        return vec![TrackDifference {
            label: String::new(),
            value: "missing workflow file".to_string(),
        }];
    ...
    let Ok(pinned_workflow) = parse_file(&pinned_workflow_path) else {
        return Vec::new();
    };
...
#[cfg(test)]
mod tests {
    use super::{edit_distance, format_number, parse_value, LoRAInfo};
    use serde_json::json;
